...
public interface UserRepository extends Neo4jRepository<User, Long> {
    @Query("create (:user{openid: $openid})")
    void createUser(@Param(value = "openid") String id);

    @Query("match (u:user),(c:course) where u.openid = $u_id  and c.no= $c_no create (u)-[r:RECOMMEND]->(c) return r")
    void updateUserRecommendCourse(@Param("u_id") String u_id,
                                   @Param("c_no") String c_no);

    @Query("match (u:user),(c:course) where u.openid = $u_id  and c.no= $c_no create (u)-[r:UNLIKE]->(c) return r")
