...
class CrawlerSession:
    ...
    async def crawl(self,courseno):
        ...
        r = await self.__session.get(f'{self.__backend}/DataQuery/QueryCourseIntro?courseNo='+courseno,
                                     ssl=SSL_CONTEXT)
        soup = BeautifulSoup(await r.text(), 'html.parser')
        ...
        # }
        # cours.append(courseInfo)
        keys = [['课程编号','no'],['课程目标','aim'], ['课程内容','content'],['教材与主要参考书','reference'], ['先修课程','requisites'], ['建议选课对象','target']]
        data = {}

        for key in keys:
            th = soup.find('th', text=key[0])
            if th:
                td = th.find_next_sibling('td')
                if td:
                    data[key[1]] = td.text.replace('\n', '')
...
async def do_crawl(output_dir, backend, username, password):
    ...
    try:
        ...
        await session.login()
        print('Process crawl...')
        df = pd.read_excel('course.xlsx', sheet_name='Result 1')

        # 获取 c_no 列数据
        c_no_data = df['c_no']

        # 将 c_no 列数据转换为列表
        c_no_list = c_no_data.tolist()
        print(c_no_list)
        for c_no in c_no_list:
            for term_id in session.term_id_list:
                await session.select_term(term_id)
                result[term_id] = await session.crawl(c_no)
            print("finish\n")
            for term_id, data in result.items():
                with open(os.path.join(output_dir, os.path.join('courseInfo', f'{c_no}.json')), 'w', encoding='utf-8') as fp:
                    print(f'Save term data to {os.path.join("courseInfo", f"{c_no}.json")}')
